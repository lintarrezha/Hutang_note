        ...
        function renderPiutang() {
            ...
            const piutangs = storage.debts.filter(debt => 
                debt.userId === storage.currentUser.id && debt.type === 'piutang'
            ...
            piutangList.innerHTML = piutangs.map(debt => `
                <div class="debt-card">
        ...
        function renderHutang() {
            ...
            const hutangs = storage.debts.filter(debt => 
                debt.userId === storage.currentUser.id && debt.type === 'hutang'
            ).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
            ...
            hutangList.innerHTML = hutangs.map(debt => `
                <div class="debt-card">
                    <div class="debt-person">${debt.person}</div>
        ...
        function updateSummary() {
            ...
            const totalPiutang = userDebts
                .filter(debt => debt.type === 'piutang')
                .reduce((sum, debt) => sum + debt.amount, 0);
            const totalHutang = userDebts
